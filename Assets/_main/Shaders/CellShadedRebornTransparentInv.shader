    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float Transparency = _Transparency_var;
                float node_3916 = Transparency;
                float3 normalLocal = lerp(Normal,float3(0,0,1),node_3916);
...
////// Emissive:
                ...
                float node_8533 = 0.0;
                float3 emissive = lerp(Emission,float3(node_8533,node_8533,node_8533),node_3916);
                ...
                float3 finalColor = emissive + lerp(((node_7200+(floor(node_4056 * node_3226) / (node_3226 - 1)*node_7026))*(_LightColor0.rgb*floor(attenuation * node_3373) / (node_3373 - 1)*_LightColor_var.rgb)),sceneColor.rgb,node_3916);
...
////// Lighting:
                ...
                float node_3373 = posterizeSteps;
                float4 _LightColor_var = UNITY_ACCESS_INSTANCED_PROP( Props, _LightColor );
                float3 finalColor = lerp(((node_7200+(floor(node_4056 * node_3226) / (node_3226 - 1)*node_7026))*(_LightColor0.rgb*floor(attenuation * node_3373) / (node_3373 - 1)*_LightColor_var.rgb)),sceneColor.rgb,node_3916);

...
//Script base de David Ville con modificaciónes de Luis Eduardo Salamán
...
public class PoolManager : MonoBehaviour
{
    ...
    public static Transform Spawn(GameObject prefab, Vector3 position, Quaternion rotation, Transform parent)
    {
        Transform temp = Spawn(prefab, position, rotation, parent);
        temp.parent = parent;
        return temp;
    }

    public static Transform SpawnWithClip(GameObject prefab, Vector3 position, Quaternion rotation, AudioClip clip, Transform parent = null)
    {
        if (pool.ContainsKey(prefab.GetInstanceID()))
        {
            bool areAnyAvailable = false;
